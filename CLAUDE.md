...
**`book/index.html`** — Scholar booking page at `/book`. Homepage font stack (Cinzel/Lato). Driven entirely by the `SCHOLARS` array at the top of its script: an entry with an empty `calendlyUrl` renders an explanatory panel and loads no third-party script; entries with a URL render Calendly's inline embed, with a scholar picker when more than one is bookable. Calendly's `widget.js` is injected lazily and only when a real URL exists, so the shipping state makes no third-party request. Keep that property — do not move the script into `<head>`.
...
**Two card types:**
- **Pinned cards** (`PINNED_DEFAULTS`, keys listed in `PINNED_KEYS`): fixed keys `sisters` / `quran` / `book` / `whatsapp` / `instagram` — stored under `aah_pinned` in localStorage. Can be hidden (`removed: true`) but not deleted. `whatsapp`/`instagram` are permanent program cards (distinct from the floating `.social-float` corner icons, which still exist separately and are not admin-managed). To add another fixed card, add it to `PINNED_KEYS` and `PINNED_DEFAULTS` — every other reference to pinned cards in `admin/index.html` reads from `PINNED_KEYS`, not a hardcoded key list. **Also seed the card's markup into `index.html` by hand in the same commit**, matching `buildPinnedCardHtml()`'s output exactly (including `target="_blank" rel="noopener"`, which it emits even for internal links). `syncFromLiveSite()` sets `removed = !liveOrder.includes(key)`, so a new key that is absent from the published page syncs as removed and the card sits hidden until someone clicks Restore. This is how `book` was added.
