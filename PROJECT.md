...
quran-reflections/
  ...
  notes/                 ← Obsidian .md files (source of truth for session content)
    ...
    2026-03-07 - ....md  ← drop Obsidian notes here
  session-1-title/
...
Each note file must follow this convention at the top (after YAML frontmatter):

```markdown
# Session Title Here

**Sat. Month Day, Year · Molana Farhat Abbas**
...
- The H1 title is used in the session list and as prev/next labels — keep it concise.
- The bold byline renders as a muted 1.05rem EB Garamond line below the title.
- The `---` separator triggers the injected 3-box navigation bar (`← Previous | Overview | Next →`).
- Do not put navigation links in the note body — prev/next/overview are generated automatically.

### What the build script does (`build_notes.py`)

- Reads all `.md` files from `quran-reflections/notes/` (ignores `README.md`)
- Strips YAML frontmatter before markdown rendering (fields like `speaker`, `series`, `session_num` are not shown on public pages)
- Both `[[Quran S-V]]` and `![[Quran S-V]]` render as full inline Arabic + translation cards. Exception: references inside heading lines (`#`), table rows (`|`), or blockquote lines (`>`) stay as plain links.
...
- Wraps output in a styled article page template
- Injects a 3-box nav bar (`← Previous | Overview | Next →`) before the first `<hr />` in the rendered content. Boundary sessions show greyed-out boxes for the missing direction.
- Writes each session to `quran-reflections/{slug}/index.html`
- Uses each note's first markdown H1 as the display title for session lists and prev/next labels
- Injects the session list between `<!-- SESSIONS-START -->` / `<!-- SESSIONS-END -->` markers inside the `program-desc` area of `quran-reflections/index.html`
- Session list entries are plain stacked links (not card UI blocks)
- `Quran Reflections - Overview.md` still builds to its own page, but is excluded from the injected homepage session link list and the prev/next chain
...
| Filename | URL |
|---|---|
| `2026-03-07 - Surah Ankabut Big Picture.md` | `/quran-reflections/2026-03-07-surah-ankabut-big-picture/` |
| `2026-04-25 - Surah Nas, Shield of the Soul.md` | `/quran-reflections/2026-04-25-surah-nas-shield-of-the-soul/` |
...
Cards (glassmorphism boxes) are restricted to `index.html` (homepage). All inner pages — session notes, verse pages, surah pages, program overview — use a plain article layout: content flows directly on the warm beige gradient with a `border-top: 2px solid var(--accent-mid)` rule as the visual separator at the top of the content area. Do not add glassmorphism card wrappers to inner pages.
